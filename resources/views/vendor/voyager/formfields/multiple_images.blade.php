    ...
    @if($images != null)
        @foreach($images as $keyId => $image)
            <div class="img_settings_container" data-field-name="{{ $row->field }}" style="float:left;padding-right:15px;">
                ...
                <img src="{{ Voyager::image( $image ) }}" data-file-name="{{ $image }}" data-realid="{{$keyId}}" data-id="{{$dataTypeContent->getKey()}}" style="max-width:200px; height:auto; clear:both; display:block; padding:2px; border:1px solid #ddd; margin-bottom:5px;">
            </div>
        @endforeach
    @endif
@endif
